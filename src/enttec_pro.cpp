...
EnttecPro::EnttecPro()
    : m_state(STATE_WAIT_START), m_label(0), m_dataLength(0), m_dataIndex(0) {
...
void EnttecPro::handleSendDmx(uint8_t universe) {
  ...
  if (channelCount < MAX_DMX_CHANNELS) {
    memset(&m_dmxBuffer[universe][channelCount], 0,
           MAX_DMX_CHANNELS - channelCount);
  }
...
// Configures Port direction on Mk2 widgets. We respond with a mock success
// packet.
// =============================================================================

void EnttecPro::handlePortAssign() {
  // Pro Mk2 port assignment response: usually just returns a status byte (0 =
  // Success)
  uint8_t status = 0;
